            ...
            if (TT.lineId == 108) { //경춘선
                ...
                if (transtation.stationCode.contains("P")) {
                    if (TT.typeName.equals("S")) {  //급행
                        if (transtation.express == 1) {
                            SubwayData sub = giveMe(TT);
                            int result = sub.stationCode.compareTo(transtation.stationCode.substring(1, 4));
    ...
    for (int i = 0; i < sm.finalPath.size(); i++) {
        ...
        while (!finalroute.isEmpty()) {
            ...
            if ((station.transfer && station.transferInfo.timeSec == 0) || prev == root) {  //출발역 or 환승역
                if(prev.transferInfo.timeSec != 0) {
                    ...
                    if(station.stationDetailId == 41 && station.lineDirection == 0) {
                ...
                if(prev.stationName.equals("응암") && station.stationName.equals("역촌")) {
                    station.schedule = databaseManager.getOneScheduleDB(prev, station, "봉화산");
                }
                else
